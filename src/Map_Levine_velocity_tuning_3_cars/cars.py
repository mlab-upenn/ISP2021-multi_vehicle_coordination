...
def execute_pure_pursuit(num, q, time_step):
    ...
    env = gym.make(
        "f110_gym:f110-v0", map=conf.map_path, map_ext=conf.map_ext, num_agents=3
    )
    ...
    planner_3 = PurePursuitPlanner(0.17145 + 0.15875, False, conf)
    ...
    while not done:
        ...
        if np.size(data["trajectory"][0]) != 0:
            ...
            nptraj_3 = np.array(trajectory[2])
            nptraj_3 = np.reshape(nptraj_3, (-1, 2))
            ...
            if "trajectory_velocity" in data:
                ...
                trajectory_velocity_3 = trajectory_velocity[2]
                ...
                nptraj_3 = np.array(trajectory_velocity_3)
                nptraj_3 = np.reshape(nptraj_3, (-1, 4))
...
#            env.renderer.update_laser_scan_obst(laser_scan_obst, 2)
        ...
        planner_3.update_paths(nptraj_3, goal_pts[2])
        ...
        speed_3, steer_3, lookahead_point_3 = planner_3.plan(
            obs["poses_x"][2],
            obs["poses_y"][2],
            obs["poses_theta"][2],
            work["tlad"],
        ...
        if curr_time_idx < len(nptraj_3) - 1:
            speed_3 = nptraj_3[curr_time_idx, 2]

#        print('car: ',2,'; time: ', laptime, 'planned velocity: ', speed_2, 'actual velocity: ', obs["linear_vels_x"][1], 'distance: ', np.sqrt((obs["poses_x"][0] - obs["poses_x"][1]) * (obs["poses_x"][0] - obs["poses_x"]
#              [1]) + (obs["poses_y"][0] - obs["poses_y"][1]) * (obs["poses_y"][0] - obs["poses_y"][1])))
#        print('car: ',3,'; time: ', laptime, 'planned velocity: ', speed_3, 'actual velocity: ', obs["linear_vels_x"][2], 'distance: ', np.sqrt((obs["poses_x"][0] - obs["poses_x"][2]) * (obs["poses_x"][0] - obs["poses_x"]
#              [2]) + (obs["poses_y"][0] - obs["poses_y"][2]) * (obs["poses_y"][0] - obs["poses_y"][2])))
        ...
        obs, step_reward, done, info = env.step(
            np.array([[steer_1, speed_1], [steer_2, speed_2], [steer_3, speed_3]])
        )
