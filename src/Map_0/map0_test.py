...
class PurePursuit:
    ...
    def get_current_waypoint(self, lookahead_distance, position, theta):
        ...
        if nearest_dist < lookahead_distance:
            lookahead_point, i2, t2 = first_point_on_trajectory_intersecting_circle(
                position, lookahead_distance, wpts, i + t, wrap=False
            )
            if i2 == None:
                return None
    ...
    def plan(self, pose_x, pose_y, pose_theta, lookahead_distance, vgain):
        ...
        lookahead_point = self.get_current_waypoint(
            ...
            if (
                ...
                if self.rrt_star == True:
                    ...
                    for j in range(len(near_set)):
                        if (
                            self.check_collision(tree[near_set[j]]) == False
                            and (
    ...
    def check_collision(self, nearest_node, new_node):
        ...
        for i in range(0, 4):
            ...
            if i == 3:
                ...
                D = car_pts_arr[i + 1]
            collision = self.ccw(A, C, D) != self.ccw(B, C, D) and self.ccw(
                A, B, C
            ) != self.ccw(A, B, D)
        return collision
...
if __name__ == "__main__":
    ...
    while not done:
        ...
        path = rrt.find_path()
        obs, step_reward, done, info = racecar_env.step(
            np.array([[steer, speed], [0.0, 0.5]])
        )
        if len(path) == 0:
            continue
