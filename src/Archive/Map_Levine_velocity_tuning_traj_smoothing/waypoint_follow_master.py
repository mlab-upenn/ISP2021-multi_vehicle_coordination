...
if __name__ == "__main__":
    ...
    while True:
        ...
        if len(trajectory_1) > 1 and len(trajectory_2) > 1:
            ...
            smoothedTraj_1 = TrajectorySmoothing(np.flip(trajectory_1,0), 2.0)
            smoothedTraj_2 = TrajectorySmoothing(np.flip(trajectory_2,0), 1.0)
            ...
            pathTraj_1 = PathTrajectory(smoothedTraj_1.get_total_path(time_step), 
                                        np.array([[0,0],[smoothedTraj_1.total_time, 1]]))
            pathTraj_2 = PathTrajectory(smoothedTraj_2.get_total_path(time_step), 
                                        np.array([[0,0],[smoothedTraj_2.total_time, 1]]))
